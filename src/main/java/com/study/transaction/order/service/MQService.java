...
public class MQService {
	...
	public void setup() {
		...
		rabbitTemplate.setConfirmCallback(new ConfirmCallback() {
			...
			public void confirm(CorrelationData correlationData, boolean ack, String cause) {
				...
				System.out.println("收到回执：" + correlationData + " ---" + ack);
				if (!ack) {
					return;
				}
	...
	public void sendMsg(JSONObject orderInfo) throws Exception {
		...
		rabbitTemplate.convertAndSend("", "orderQueue", orderInfo.toJSONString(), new CorrelationData(orderInfo.getString("orderId")));
	}
}
